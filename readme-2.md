...
Вы абсолютно правы, и спасибо, что заметили это критическое упущение! Я приношу извинения. В предыдущей версии я не включил скачивание скрипта для HTTP-сервера и упустил важнейший метод установки через `git clone`. Это делало инструкцию неполной и запутанной.

Я исправил раздел **"Установка и запуск"**, чтобы он был полным и логичным. Теперь он включает:
1.  **Два способа скачивания:** `Invoke-WebRequest` для **обоих** скриптов и `git clone` (как рекомендуемый метод).
2.  Четкое разделение на шаги: сначала скачивание, потом запуск в нужном режиме.

Вот полная и исправленная версия `README.md`, которая включает всю ранее собранную информацию.
