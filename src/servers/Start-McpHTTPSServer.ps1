...
function Load-ServerConfig {
    ...
    if (-not (Test-Path $FullPath)) {
        Write-Host "Файл конфигурации не найден: $FullPath. Используется конфигурация по умолчанию." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "Ошибка чтения конфигурации: $($_.Exception.Message). Используется конфигурация по умолчанию." -ForegroundColor Red
        return $DefaultConfig
    }
}
...
$script:ServerConfig = @{
    Port = $Port
    Host = $ServerHost
    MaxConcurrentRequests = if ($script:ServerConfigFromFile.Http.MaxConcurrentRequests) { 
        $script:ServerConfigFromFile.Http.MaxConcurrentRequests 
    } else { 
        10 
    }
    TimeoutSeconds = if ($script:ServerConfigFromFile.MaxExecutionTime) { 
        $script:ServerConfigFromFile.MaxExecutionTime 
    } else { 
        300 
    }
    Name = $script:ServerConfigFromFile.Name
    Version = $script:ServerConfigFromFile.Version
    Description = $script:ServerConfigFromFile.Description
    LogLevel = $script:ServerConfigFromFile.LogLevel
}

$script:RestrictedCommands = if ($script:ServerConfigFromFile.Security.RestrictedCommands) {
...
function Invoke-PowerShellScript {
    ...
    try {
        ...
        if ($completed) {
            ...
            $maxSize = $script:ServerConfigFromFile.Security.MaxOutputSize
